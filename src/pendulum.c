...
void update_pendulum(Pendulum *p, double dt, double pixels_per_meter, int screen_width, int screen_height) {
    ...
    // 1. Physics Update (RK4) - Uses only local variables
    compute(
        theta1, theta2,
        omega1, omega2,
        m1, m2,
    ...
    // 2. Write-back state - Uses only local variables
    theta1 = new_theta1;
    theta2 = new_theta2;
    omega1 = new_omega1;
    omega2 = new_omega2;
    ...
    // 3. Trail History Update - Use local variables/simple pointer access for trail
